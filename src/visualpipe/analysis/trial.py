...
class Trial(object):
    ...
    def find_responsive_rois(self, save_dir=None, folder_prefix=None, dt_min:float=0.2, with_plots=False):
        """
                        ...
                        if len(roi_trial) >= min_nb_trials :
                            ...
                            if np.mean(r_dist) > perc_th :
                                if positive :
                                    ...
                                    responsive_roi.append((-1, pval))
                                if with_plots :
                                    self.plot_hist_reliability(r_dist, r_null_dist, perc_th, pval, 'skyblue', i, roi_idx, save_dir, folder_prefix)
                            else :
                                responsive_roi.append((0, pval))
                                if with_plots :
                                    self.plot_hist_reliability(r_dist, r_null_dist, perc_th, pval, 'thistle', i, roi_idx, save_dir, folder_prefix)
                        else :
                            if positive :
    ...
    def compute_reliability(self, roi_trials_traces, n_samples=1):
        ...
        return corr_list, null_corr_list
    ...
    def plot_hist_reliability(self, r_dist, r_null, perc, p_value, color:str, stimuli_id:int, neuron_idx:int, save_dir:str, folder_prefix:str=''):
        stimuli_name = self.visual_stim.protocol_df['name'][stimuli_id]
        ...
        ax.axvline(np.mean(r_dist), color=color, linestyle='--', label='r')
    ...
    def save_trials(self, save_dir, filename):
        trials_info = {
            ...
            "trial_fluorescence": self.trial_fluorescence,
            "pre_trial_fluorescence" : self.pre_trial_fluorescence,
            "post_trial_fluorescence" : self.post_trial_fluorescence,
