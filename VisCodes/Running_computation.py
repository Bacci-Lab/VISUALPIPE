...
def compute_speed(base_path, position_smoothing = 10e-3, #s
    ...
    print("this is acq_freq", acq_freq)
    A, B = process_binary_signal(binary_signal)

    position = compute_position_from_binary_signals(A,
                                                    B) * 2. * np.pi * radius_position_on_disk / rotoencoder_value_per_rotation
    ...
    new_t, speed = resample_signal(speed,
